...
<div class="the-menu-box">
  <transition name="fade-width" mode="out-in">
    <div class="the-menu-logo" @click="gotoIndex()" :key="is_collapse">{{is_collapse?'WL':'wl-admin'}}</div>
  </transition>
...
import { getMenuDataApi } from "@/api/menu"; // 导入api接口
...
export default {
  ...
  computed: {
    ...
    // 导入vuex菜单数据，菜单折叠状态
    ...mapGetters(["menu", "is_collapse", 'current'])
  },
  watch: {
    is_collapse(){
      this.scrollDestroy() 
  ...
  methods: {
    // 获取菜单列表
    getMenuData() {
      getMenuDataApi().then(({ data }) => {
        if(data.code === this.GLOBAL.code.ok){
